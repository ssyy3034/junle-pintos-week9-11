...
void exec_children(const char *child_name, pid_t pids[], size_t child_cnt)
{
    ...
    for (i = 0; i < child_cnt; i++)
    {
        ...
        {
            CHECK(pids[i] != PID_ERROR, "exec child %zu of %zu: \"%s\"", i + 1, child_cnt, cmd_line);
        } else
        {
            exec(cmd_line);
        }
...
void wait_children(pid_t pids[], size_t child_cnt)
{
    ...
    for (i = 0; i < child_cnt; i++)
    {
        ...
        CHECK(status == (int)i, "wait for child %zu of %zu returned %d (expected %zu)", i + 1, child_cnt, status, i);
    }
}
...
void check_file_handle(int fd, const char *file_name, const void *buf_, size_t size)
{
    ...
    while (ofs < size)
    {
        ...
        if (ret_val != block_size)
            fail("read of %zu bytes at offset %zu in \"%s\" returned %zu", block_size, ofs, file_name, ret_val);
