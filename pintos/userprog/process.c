...
/* FILE_NAME에서 로드한 "initd"라는 이름의 첫 번째 유저랜드 프로그램을 시작한다.
 * 새 스레드는 process_create_initd()가 반환되기 전에
 * 스케줄되어 실행되거나, 심지어 종료될 수도 있다.
 * initd의 스레드 ID를 반환하며, 스레드를 생성할 수 없으면 TID_ERROR를 반환한다.
 * 이 함수는 반드시 한 번만 호출되어야 한다는 점에 주의하라. */
...
/* FILE에서 OFS 오프셋부터 시작하는 세그먼트를
 * 가상 주소 UPAGE에 로드한다.
 * 전체적으로 READ_BYTES + ZERO_BYTES 만큼의 가상 메모리를
 * 다음과 같은 방식으로 초기화한다:
 *
 * - UPAGE에서 시작하는 READ_BYTES 바이트는
 *   FILE의 OFS 오프셋부터 읽어와 채운다.
 *
 * - UPAGE + READ_BYTES 위치부터 ZERO_BYTES 바이트는
 *   0으로 채운다.
 *
 * 이 함수에 의해 초기화된 페이지들은,
 * WRITABLE이 true이면 사용자 프로세스가 쓸 수 있어야 하고,
 * 그렇지 않다면 읽기 전용이어야 한다.
 *
 * 메모리 할당 에러나 디스크 읽기 에러가 발생하지 않고
 * 성공적으로 수행되면 true를 반환하고,
 * 그렇지 않으면 false를 반환한다. */
static bool
...
/* USER_STACK 위치에 0으로 초기화된 페이지를 매핑해서
 * 최소한의 스택을 생성한다. */
static bool
...
/* 사용자 가상 주소 UPAGE를 커널 가상 주소 KPAGE에 매핑하는 항목을
 * 페이지 테이블에 추가한다.
 * WRITABLE이 true이면, 사용자 프로세스가 해당 페이지를 수정할 수 있고,
 * 그렇지 않으면 읽기 전용이다.
 * UPAGE는 이미 매핑되어 있지 않아야 한다.
 * KPAGE는 보통 palloc_get_page()로 user pool에서 얻은 페이지여야 한다.
 * 성공하면 true를 반환하고,
...
install_page(void *upage, void *kpage, bool writable)
{
	...
	/* 해당 가상 주소에 이미 페이지가 존재하지 않는지 확인한 뒤,
	 * 우리 페이지를 그 주소에 매핑한다. */
