...
static bool pt_for_each(uint64_t *pt, pte_for_each_func *func, void *aux, unsigned pml4_index, unsigned pdp_index,
                        unsigned pdx_index)
{
    for (unsigned i = 0; i < PGSIZE / sizeof(uint64_t *); i++)
    {
        ...
        if (((uint64_t)*pte) & PTE_P)
        {
            void *va = (void *)(((uint64_t)pml4_index << PML4SHIFT) | ((uint64_t)pdp_index << PDPESHIFT) |
...
/* pml4에서 사용자 가상 주소 UADDR에 대응되는 물리 주소를 찾아낸다.
 * 그 물리 주소에 대응되는 커널 가상 주소를 반환하며,
 * 만약 UADDR가 매핑되어 있지 않다면 null 포인터를 반환한다. */
