...
/* 빈 페이지 하나를 할당해서 그 커널 가상 주소를 반환한다.
   PAL_USER 플래그가 설정되어 있으면, 유저 풀(user pool)에서 페이지를 가져오고,
   설정되어 있지 않으면 커널 풀(kernel pool)에서 가져온다.
   FLAGS에 PAL_ZERO가 설정되어 있으면, 할당된 페이지를 0으로 채운다.
   만약 사용 가능한 페이지가 없다면 null 포인터를 반환한다.
   다만 FLAGS에 PAL_ASSERT가 설정되어 있는 경우에는
   null을 반환하는 대신 커널이 패닉(panic)을 일으킨다. */
